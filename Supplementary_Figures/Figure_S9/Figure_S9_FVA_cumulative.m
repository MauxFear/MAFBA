%% FIGURE S9 - FLUX VARIABILITY ANALYSIS (FVA): HOMOGENEOUS APPROACH
% Shows cumulative distributions of flux variability comparing:
%   - Homogeneous approach at multiple gamma values (0.20, 0.25, 0.30, 1.00)
%   - irreversible iML1515 (no enzymatic constraints, FBA only)
%
% Data source: data/outputs/FVA_Results/
% Outputs: Four panels (A-D) saved to output/
...
%% SETUP
figurePrefix = 'iML1515_Homog_FVA';

selected_models = {'Homog', 'irriML1515'};
models_labels   = {'Homogeneous approach', 'irreversible iML1515'};

uptake_bound    = 1000;
FVA_percentage  = 0;
analysis_types  = {'all_reactions', 'membrane', 'nonmembrane'};
...
combined_labels = {
    'Homogeneous approach, \gamma = 0.20',
    'Homogeneous approach, \gamma = 0.25',
    'Homogeneous approach, \gamma = 0.30',
...
for m = 1:length(analysis_types)
    ...
    current_data = all_data(strcmp(all_data.Analysis_Type, analysis_type), :);
    ...
    panel_tag     = sprintf('panel_%c', char(64 + m));
...
if ~isempty(reactionSubsystemMap)
    ...
    if ~isempty(panel_d_data)
        ...
        if ~isempty(topData) && ~isempty(botData)
            ...
            col_top  = sscanf('40E0D0','%2x%2x%2x',[1 3])/255; % Turquoise (γ=0.20)
            ...
            for si = 1:numel(pick)
                ...
                if ~isempty(vt)
                    ...
                    X  = [xi(:).' fliplr(xi(:).')];
                    Y  = [ (yc(:).'+f(:).') fliplr(yc(:).') ];
                    patch(X, Y, col_top, 'FaceAlpha', 0.5, 'EdgeColor', col_top*0.8);
                ...
                scatter(vTop_all, si + 0.18 + jit, 10, 'filled', ...
                    'MarkerFaceAlpha',0.25, 'MarkerFaceColor', col_top, 'MarkerEdgeColor','none');
            ...
            h1 = plot(nan,nan,'s','MarkerFaceColor',col_top, 'MarkerEdgeColor','none');
            h2 = plot(nan,nan,'s','MarkerFaceColor',col_blue,'MarkerEdgeColor','none');
            lg = legend([h1 h2], ...
                {sprintf('Homogeneous approach, \\gamma = %.2f', gamma_top), 'No Membrane constraint, \gamma = 1.00'}, ...
...
fprintf('    - %s (multiple gamma values)\n', selected_models{1});
fprintf('    - %s (gamma=1.0 only)\n', selected_models{2});
fprintf('  Uptake bound: %d mmol gDW^{-1} h^{-1}\n', uptake_bound);
fprintf('  Total panels: 4 (A, B, C, D)\n');
fprintf('  Total files: 4 panels x 2 formats = 8 files\n');
