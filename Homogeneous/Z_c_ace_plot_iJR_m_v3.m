...
for f=1:length(gamma_v)
    ...
    gamma_string = ['\gamma = ', str , '    \phi Y ' ];
    title({'MAFBA: Flux profile vs growth rate','varying w_{Cc}', gamma_string})
    ...
    for u=1:length(u_v)
        ...
        for i=1:Npoints
            ...
            memb(i)=0.25*sol_M(i,f).x(memb_r);
        ...
        str_u = ['\phi Y ', utr, '% :']; 
        plot(T.gr,T.ac,markers{u},'Color',color{u},'LineWidth',1, 'markerfacecolor', color{u}, 'MarkerSize', 6, 'DisplayName',[str_u,' Ex_{ac} flux'] );
        plot([gr_ac,gr_ac], [0, 100],'--','Color',color{u},'LineWidth',1, 'DisplayName',[str_u,' \lambda_{ac}']);
...
%         max_memb(u) = max(memb);
    ...
    lgd = legend();
    set(lgd, 'Box', 'off', 'Location', 'west','Fontsize', 14,...
...
%     membmax = max(max_memb);
    ...
    hold off;
    filename = ['M_Y_wC_ace_g_', str,'.png'];
