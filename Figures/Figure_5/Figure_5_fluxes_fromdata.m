...
% Shows how metabolic fluxes change with parameter sensitivity (change factors)
% across different gamma values
%
% Data source: Sensitivity analysis from data collection
% Outputs: Flux plots for combinations of gamma and change factor values
...
if ~isfile(datafilename)
    fprintf('Specified data file not found. Looking for any CSV file in the directory...\n');
    ...
    if isempty(files)
        ...
        datafilename = fullfile(datafolder, files(1).name);
...
% Get unique gamma values from data
all_gamma_values = unique(tableData.(gamma_col));
fprintf('Found %d unique gamma values in data\n', length(all_gamma_values));

% Filter to specific gamma values for Figure 5
gamma_values = [1.0, 0.3, 0.25, 0.23];
fprintf('Using %d gamma values for Figure 5: %s\n', length(gamma_values), mat2str(gamma_values));

% Get unique change factors from data
all_change_factors = unique(tableData.(cf_col));
fprintf('Found %d unique change factors in data\n', length(all_change_factors));

% Filter to CF = 1.0 only (baseline, no parameter perturbation)
change_factors = [1.0];
fprintf('Using change factor = %.1f (baseline)\n', change_factors(1));

% Output naming
figurePrefix = ['iML1515_MAFBA'];

%% GENERATE FIGURES
fprintf('\n=== GENERATING FIGURE 5 PANELS ===\n');
fprintf('Selected gamma values: %s\n', mat2str(gamma_values));
fprintf('Selected change factor: %.1f\n', change_factors(1));
...
for g = 1:length(gamma_values)
    ...
    for c = 1:length(change_factors)
        ...
        if height(T) < 2
            fprintf('    ⚠ Skipping: Not enough data points (found %d)\n', height(T));
            continue;
        end
        ...
        filename_svg = fullfile(output_folder, sprintf('%s_flux_g%03d_cf%03d.svg', figurePrefix, round(gamma_val * 100), round(change_factor * 100)));
        saveas(fig, filename_svg, 'svg');
        
        % Also save as PNG for easier viewing
        filename_png = fullfile(output_folder, sprintf('%s_flux_g%03d_cf%03d.png', figurePrefix, round(gamma_val * 100), round(change_factor * 100)));
...
fprintf('  Change factor: %.1f (baseline)\n', change_factors(1));
fprintf('  Total files: %d gamma values × 2 formats = %d files\n', ...
    length(gamma_values), length(gamma_values) * 2);
...
function fig = get_plot(T, gamma_string, gr_ac, reaction_map)
    ...
    plot(growth_rate, T.(reaction_map.memb), '-', 'Color', 'k', 'LineWidth', 4, 'HandleVisibility', 'off');
    ...
    plot(growth_rate, T.(reaction_map.memb), '-', 'Color', color{4}, 'LineWidth', 4, 'HandleVisibility', 'off');
